# AGENTS.md — Phase 1B LMAP Study Wiki Schema
...
- **Subject:** The Phase 1B Lifestyle Medicine and Prevention (LMAP) module at Imperial College London — the Year 2 LMAP course covering lifestyle medicine, behaviour change, epidemiology, global health, health coaching, and related therapeutic / population interventions.
...
- **Out of scope:** Material from other Phase 1B modules (e.g. BRS, CSI, PCH), Phase 1A LMAP content that does not clarify Year 2 teaching, university administration, and medical detail unrelated to the Phase 1B LMAP curriculum, unless it directly clarifies an in-scope topic. If a source is broader than the wiki's scope, extract only the in-scope facts; note out-of-scope content in the source page's "Out of scope notes" section but do not propagate it into entity/concept pages.
...
For things with **stable identity** within the domain. Use these entity types consistently: `lifestyle-theme`, `condition`, `risk-factor`, `intervention`, `metric`, `tool`, `organisation`, and `stakeholder`.
...
## Properties
- **Type:** <lifestyle-theme | condition | risk-factor | intervention | metric | tool | organisation | stakeholder>
- **Owner / Operator:** ...
...
For **ideas, frameworks, processes, and recurring themes** within the domain — for example behaviour-change models (e.g. COM-B), levels of prevention, epidemiological reasoning, social determinants of health, lifestyle medicine principles, health coaching approaches, population / global health themes, and study themes.
...
The template was instantiated for the Phase 1B Lifestyle Medicine and Prevention (LMAP) module at Imperial College London on 2026-07-19. The wiki is ready for its first ingest per §5.1.
